...
private:
  ...
  void set_initial_watches(clause * cl, int local_search = 0);
  void check_clauses(const std::vector<clause *> & clauses, int local_search = 0);
  void process_watch_list(const expression & l);
  void process_watch_list(const expression & l, unsigned local_search);
