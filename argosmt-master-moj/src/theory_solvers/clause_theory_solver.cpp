...
void clause_theory_solver::add_locked_clause(clause * cl)
{
...
void clause_theory_solver::set_reason_clause(const expression & l, clause * cl, unsigned index)
{
...
unsigned clause_theory_solver::find_initial_watches(clause * cl, unsigned & w0, unsigned & w1)
{
  unsigned w = 0;
  w0 = 0;
  w1 = 1;
...
void clause_theory_solver::set_initial_watches(clause * cl, int local_search )
{
  ...
  // special case -- unit clause!!
      ...
      else
	{
	  ...
	  watch_list & w0_list = _data.get_data((*cl)[w0])->get_watch_list();
	  w0_list.push_back(cl);

	  cl->set_watch_pos(1, w1);
	  watch_list & w1_list = _data.get_data((*cl)[w1])->get_watch_list();
	  w1_list.push_back(cl);
	}
    }
}

void clause_theory_solver::check_clauses(const std::vector<clause *> & clauses, int local_search )
{
  for(unsigned i = 0; i < clauses.size(); i++)
    {
      //std::cout << "Clause: " << *clauses[i] << std::endl;
      set_initial_watches(clauses[i], local_search);
    }
}

void clause_theory_solver::process_watch_list(const expression & lp)
{
  watch_list & watch_list = _data.get_data(lp)->get_watch_list();
  unsigned k = 0;
...
void clause_theory_solver::process_watch_list(const expression & lp, unsigned local_search)
{
  watch_list & watch_list = _data.get_data(lp)->get_watch_list();
  unsigned k = 0;
...
bool clause_theory_solver::find_alternative_watch(clause * cl, unsigned cw)
{
  ...
  for(unsigned i = 0; i < cl->size(); i++)
      ...
      if(!_solver.get_trail().is_false((*cl)[i]))
	{
	  ...
	  _data.get_data((*cl)[i])->get_watch_list().push_back(cl);
	  return true;
	}
    }
  return false;
...
bool clause_theory_solver::is_clause_true(const trail & tr, clause * cl)
{
  for(unsigned i = 0; i < cl->size(); i++)
...
void clause_theory_solver::remove_clauses(const std::vector<clause *> & learnt_clauses,
					  unsigned number_of_clauses)
{
  for(unsigned i = learnt_clauses.size() - number_of_clauses;
	...
	{
	  watch_list & wl = _data.get_data(cl->get_watch_literal(j))->get_watch_list();
...
void clause_theory_solver::new_level()
{
...
void clause_theory_solver::backjump(unsigned level)
{
...
void clause_theory_solver::check_and_propagate(unsigned layer)
{
...
void clause_theory_solver::check_and_propagate(unsigned layer, unsigned local_search)
{
  if(layer >= _num_of_layers)
    return;
  ...
  _check_and_prop_time_spent.start();
    ...
    if(!_initial_clauses_checked)
    {
      check_clauses(_solver.get_initial_clauses(), local_search);
      _initial_clauses_checked = true;
    }
  if(_just_backjumped)
    {
      if(!_dead_clauses.empty())
	{
	  std::vector<clause *> temp;
	  temp.swap(_dead_clauses);
	  check_clauses(temp, local_search);
	}
      _just_backjumped = false;
    }
    //  std::cout << "BOOL THEORY: " << layer << std::endl;
...
expression clause_theory_solver::canonize_expression(const expression & e)
{
  expression ep;
  bool negative;
...
void clause_theory_solver::get_literal_pair(const expression & l, expression & l_pos, expression & l_neg)
{
...
bool clause_theory_solver::verify_assignment(const trail & tr)
{
...
void clause_theory_solver::get_model(const expression_vector & exps)
{
