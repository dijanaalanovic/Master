...
void arithmetic_theory_solver::process_assertions(int local_search )
{
...
void arithmetic_theory_solver::process_assertion(const expression & l, int local_search )
{
  ...
  if(l_data->has_constraint_data())
	...
	{
	case B_UPPER:
	  assert_upper(l, local_search);
	  break;
	case B_LOWER:
	  assert_lower(l, local_search);
	  break;
	case B_BOTH:
...
void arithmetic_theory_solver::assert_upper(const expression & l, int local_search)
{
  ...
  if(is_fixed_by_constraints(unknown))
    {
      ...
      c_both.bound = bound;
      c_both.type = B_BOTH;
      propagate_weaker_constraints(unknown, c_both, local_search);
    }
  else
    propagate_weaker_constraints(unknown, l_data->get_constraint(), local_search);
}

void arithmetic_theory_solver::assert_lower(const expression & l, int local_search)
{
...
void arithmetic_theory_solver::assert_both(const expression & l, int local_search)
{
...
void arithmetic_theory_solver::assert_split(const expression & l, int local_search)
{
  if(_solver.get_trail().get_source_theory_solver(l) != this)
    {      
      _disequality_asserted = true;
      ...
      const expression & unknown = l_data->get_unknown();
      const constraint & cst = l_data->get_constraint();
      propagate_weaker_constraints(unknown, cst, local_search);
    }
}
...
void arithmetic_theory_solver::check_and_propagate(unsigned layer, unsigned local_search)
{
  if(layer >= _num_of_layers)
    return;
  ...
  if(_solver.all_literals_assigned() || (layer >= 1 && _check_count >= _check_period))
    {
      do {
	...
	if(!check(bu))
	...
	else
	  {
	    ...
	    if(++_bound_refinements_count >= _bound_ref_period)
	      {
		propagate_bound_refinements(local_search);
		_bound_refinements_count = 0;
	      }	   
	  }
  ...
  assert(_current_assertion_pos == _solver.get_trail().size());
  _check_and_prop_time_spent.acumulate();
}
...
void arithmetic_theory_solver::propagate_bound_refinements(int local_search)
{
  if(!_bound_changed)
    return;
  ...
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
	...
	{
	  lo_cst.type = B_BOTH;
	  propagate_weaker_constraints(bu, lo_cst, local_search);
	}
      else
	{
	  if(up_cst.bound.infinity == I_NONE)
	    propagate_weaker_constraints(bu, up_cst, local_search);
  ...
  for(unknown_set::const_iterator it = _non_basic_unknowns.begin(), it_end = _non_basic_unknowns.end(); it != it_end; ++it)
      ...
      if(lo_cst.bound == up_cst.bound)
	{
	  ...
	  propagate_weaker_constraints(nbu, lo_cst, local_search);
	}
      else
	{
	  if(up_cst.bound.infinity == I_NONE)
	    propagate_weaker_constraints(nbu, up_cst, local_search);
	  if(lo_cst.bound.infinity == I_NONE)
	    propagate_weaker_constraints(nbu, lo_cst, local_search);
	}      
    }      
...
void arithmetic_theory_solver::propagate_weaker_constraints(const expression & unknown, const constraint & c, int local_search)
{
...
void arithmetic_theory_solver::check_equalities(int local_search)
{
      ...
      else
	{
     ...
     if(!local_search){
      if(_solver.is_conflict() || !is_poly_fixed_by_constraints(_all_unknowns[i]))
        ...
        if(!is_poly_fixed_by_constraints(_all_unknowns[i]))
	    {
	     continue;
	    }
     }
